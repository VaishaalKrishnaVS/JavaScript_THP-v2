# JavaScript Closures, Scope, and Execution Context Challenges
...
## Challenge 3
Examine the code for the outer function. Notice that we are returning a function and that function is using variables that are outside of its scope. Now we are going to create a function `addByX` that returns a function that will add an input by x.
...
## Challenge 6
Write a function `delay` that accepts a callback as the first parameter and the wait in milliseconds before allowing the callback to be invoked as the second parameter. Any additional arguments after wait are provided to the function when it is invoked. **HINT:** Research `setTimeout()`.
...
## Challenge 19
Create a function `blackjack` that accepts an array (which will contain numbers ranging from 1 through 11), and returns a DEALER function. The DEALER function will take two arguments (both numbers), and the
